...
fig3 = px.pie(df, values='Flights', names='DistanceGroup', 
             title='Distance group proportion by flights')
fig4 = px.bar(df, x='Flights', y='DistanceGroup', orientation='h',
             title='Flights per Distance group')

# Creating initial state of interactive charts
# Ideally I would like to not do this, all this is basically the function
# Hopefully Dash will be samrt enough to see the inital state of the button is at 2014 and use the 
# callbakc function to build this chart, just not sure how itll like figure=fig1 to be empty initially
default_year=2014
...
app.layout = html.Div(children=[html.H1('Interactive Dashboard'),
                                ...
                                html.Div([
                                    ...
                                    html.Div([
                                        'Input:', dcc.Input(id='input-yr-1', value=2014),
                                    ]),
                                    html.Br(),
                                    html.Br(),
                                ...
                                html.Div([
                                    ...
                                    html.Div([
                                        'Input:', dcc.Input(id='input-yr-2', value=2014),
                                    ]),
                                    html.Br(),
                                    html.Br(),
...
def month_avg_delay_yr(yr):
    ...
    return px.scatter(df_avg_delay_month, x='Month', y='DepDelay')

if __name__=='__main__':
    app.run_server(port=8053, host='localhost', debug=True)
